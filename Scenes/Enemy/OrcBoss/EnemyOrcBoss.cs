...
public class EnemyOrcBoss : KinematicBody2D, IEnemy
{
	...
	public void _Dead()
	{
		...
		coin.Position = this.GetPosition() + new Vector2(-100, 0);

		co = (PackedScene)ResourceLoader.Load("res://Scenes/Coins/Coin.tscn");
		coin = (Coin)co.Instance();
		GetParent().AddChild(coin);
		coin.Connect("coin_collected", hud, nameof(hud._on_Coin_coin_collected));
		coin.Position = this.GetPosition() + new Vector2(100, 0);
	}
