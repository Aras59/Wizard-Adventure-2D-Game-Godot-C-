...
public class EnemyOgreScript : KinematicBody2D, IEnemy
{
	...
	public override void _PhysicsProcess(float delta)
	{
		if (!isDead && !isHurt && !isHitting)
		{
			if (canWalk)
			{
				...
				velocity.x = SPEED * direction;
				if (!isRunning)
					((AnimatedSprite)GetNode("Ogre")).Play("Walk");
				else
					((AnimatedSprite)GetNode("Ogre")).Play("Running");
			}
			else
			{
				((AnimatedSprite)GetNode("Ogre")).Play("Idle");
			}
	...
	private void _Rotate()
	{
		...
		if (direction == -1)
		{
			((AnimatedSprite)GetNode("Ogre")).FlipH = true;
		}
		else
		{
			((AnimatedSprite)GetNode("Ogre")).FlipH = false;
		}
	}
	...
	public void _Dead()
	{
		...
		((AnimatedSprite)GetNode("Ogre")).Play("Dying");
		soundDie.Play();
	...
	public void _Hurt(float damage)
	{
		...
		{
			isHurt = true;
			((AnimatedSprite)GetNode("Ogre")).Stop();
			((AnimatedSprite)GetNode("Ogre")).Play("Hurt");
			soundHurt.Play();
		}
	}
	...
	public void _Hit()
	{
		isHitting = true;
		((AnimatedSprite)GetNode("Ogre")).Stop();
		((AnimatedSprite)GetNode("Ogre")).Play("Attack");
		soundAttack.Play();
		Throw();
	...
	private void Throw()
	{
		...
		rock.GlobalPosition = GlobalPosition + where;
